...
@testset "push/delete_extended_param!" begin
    ...
    @testset "push creates param chain when needed" begin
        @testset "impossible fill type" begin
            g = _make_group(; used=3, OFFSET=Int16[0,0])
            @test_throws "safely filled" push_extended_param!(g, :OFFSET, 1)

            g = _make_group(; used=255, OFFSET=Int16[0,0])
            @test_throws "safely filled" push_extended_param!(g, :OFFSET, 1)
        end

        @testset "adding one prior parameter" begin
            ...
            g = _make_group(; used=255*2+1, UNITS=fill("mV", 255))
            # UNITS2 doesn't exist. Pushing should add UNITS2 (255 empties) + UNITS2 (value)
            push_extended_param!(g, :UNITS, "mN")
            @test haskey(g, :UNITS2)
            @test length(g[:UNITS2]) == 255
            @test all(==(""), g[:UNITS2])
            @test haskey(g, :UNITS3)
            @test g[:UNITS3] == ["mN"]
        end

        @testset "adding multiple prior parameters" begin
            g = _make_group(; used=255*2+1)
            # UNITS2 doesn't exist. Pushing should add UNITS2 (255 empties) + UNITS2 (value)
            push_extended_param!(g, :UNITS, "mN")
            @test haskey(g, :UNITS)
            @test haskey(g, :UNITS2)
    ...
    @testset "push filling parameter values" begin
        g = _make_group(; used=3, LABELS=["X", "Y"])
    ...
    @testset "delete is no-op for missing param or out-of-range pos" begin
