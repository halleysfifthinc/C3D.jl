...
Entries are cascaded forward to maintain 255-entry alignment as needed
(e.g., LABELS2[1] shifts into LABELS[255], etc.).

Must be called before decrementing USED.
"""
function delete_extended_param!(group, base::Symbol, pos::Int)
    ...
    mult, local_pos = (pos-1)÷255, mod1(pos, 255) # divrem ish

    mult + 1 > length(all_keys) && return
    arr = group[all_keys[mult + 1]]
    local_pos > length(arr) && return
    deleteat!(arr, local_pos)

    # Cascade: pull first entry from each subsequent overflow param
    while mult + 1 < length(all_keys)
        next_arr = group[all_keys[mult + 2]]
...
Must be called before decrementing USED.
"""
function push_extended_param!(group, base::Symbol, value)
    pos = group[Int, :USED] + 1
    all_keys = get_extended_parameter_names(group, base)
    mult, local_pos = (pos-1)÷255, mod1(pos, 255) # divrem ish

    # Need new param(s) (e.g. LABELS → LABELS2, or LABELS itself if absent)
    if mult ≥ length(all_keys)
        # Fill any missing params in the chain with full-length empty arrays
        for m in length(all_keys):mult-1
            key = _extended_key(base, m)
            haskey(group, key) && continue
            group[key] = Parameter(key, "", fill("", 255); gid=gid(group))
        end
        # Create the target param with just the value
        target_key = _extended_key(base, mult)
        if !haskey(group, target_key)
            group[target_key] = Parameter(target_key, "",
                String[]; gid=gid(group))
        end
        push!(group[target_key], value)
        return
    end

    arr = group[all_keys[mult + 1]]
    if length(arr) < local_pos
